...
library(readxl)
MrTrash = read_excel("./dataset/202207 Trash Wheel Collection Data.xlsx", sheet = 'Mr. Trash Wheel', range = "A2:N549")
```
...
``` r
# load the dataset, choose the range to skip the figure in the first row
Professor = read_excel("./dataset/202207 Trash Wheel Collection Data.xlsx", sheet = 'Professor Trash Wheel', range = "A2:M96")
...
``` r
# load the dataset, choose the range to skip the figure in the first row
Gwynnda = read_excel("./dataset/202207 Trash Wheel Collection Data.xlsx", sheet = 'Gwynnda Trash Wheel', range = "A2:K108")
...
**Combine these with the Mr. Trash Wheel dataset to produce a single
...
# make the total table tidy
total_trash = total_trash %>%
  dplyr:::select(dumpster, year, month, day, weight_tons, volume_cubic_yards, plastic_bags, everything())
...
The number of observations in the resulting dataset is 747 . The key
variables such as `year`, `month`, `day`, represent the record time.
`dumpster` represents the “id”. `weight` is the variable that represents
the weight of the trash which we used to calculate the home being
powered. `home_powered`, as mentioned before, is the variable that
represents the amount of the home being powered by the trash. `source`
represents which sheet the data comes from. Other variables focus on
different trash being collected.
...
483 participants were recruited. 97 develop MCI.
...
For those develop MCI, the average baseline age is 65.6113402.
...
The proportion of women in the study who are APOE4 carriers is 0.6521739
.
...
mci_amyloid = mci_amyloid %>%
  rename(id=study_id,
         ...
         ratio_time_2_year = time_2,
         ratio_time_4_year = time_4,
         ratio_time_6_year = time_6,
         ratio_time_8_year = time_8)
```

For this dataset, we also need to skip the first row since it’s also
note. It totally has 487 observations. It contains key variables such as
ratio changing along 0(which is baseline), 2, 4, 6, 8 years.
...
only_in_baseline = setdiff(mci_baseline$id, mci_amyloid$id)

only_in_amyloid = setdiff(mci_amyloid$id, mci_baseline$id)
```

There are several participants only appear in baseline dataset :
participants with id 14, 49, 92, 179, 268, 304, 389, 412.

There are several participants only appear in amyloid dataset :
participants with id 484, 485, 486, 487, 488, 489, 490, 491, 492, 493,
494, 495.
...
combined_dataset = inner_join(mci_baseline, mci_amyloid, by = "id")

head(combined_dataset)
```

    ##   id current_age    sex education       apoe4 age_at_onset ratio_baseline
    ...
    ##   ratio_time_2_year ratio_time_4_year ratio_time_6_year ratio_time_8_year
...
There are 475 participants appear in both dataset. The result table
contains the infos of the participant’s id, age, sex, education, apoe4
status, MCI development, change based on time.
