...
For this dataset, we also need to skip the first row since it's also note. It totally has `r nrow(mci_amyloid)` observations and `r ncol(mci_amyloid)` variables. It contains key variables such as ratio changing along 0(which is baseline), 2, 4, 6, 8 years.
...
There are totally `r nrow(amyloid_only)` participants only appear in amyloid dataset : participants with id `r only_in_amyloid_id`.
