...
The proportion of women in the study who are APOE4 carriers is `r num_carrier_female/num_female` .
...
For this dataset, we also need to skip the first row since it's also note. It totally has `r nrow(mci_amyloid)` observations. It contains key variables such as ratio changing along 0(which is baseline), 2, 4, 6, 8 years.
...
There are several participants only appear in baseline dataset : participants with id `r only_in_baseline`.

There are several participants only appear in amyloid dataset : participants with id `r only_in_amyloid`.
...
There are `r nrow(combined_dataset)` participants appear in both dataset. The result table contains the infos of the participant's id, age, sex, education, apoe4 status, MCI development, change based on time.
