...
For this dataset, we also need to skip the first row since it's also note. We apply pivot_longer( ) to make the dataset more readable with time and ratio obviously shown. It totally has `r nrow(mci_amyloid)` observations (where an id has 5 rows) and `r ncol(mci_amyloid)` variables. It contains key variables such as id, time and ratio.
